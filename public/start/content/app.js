...
$("document").ready( function() {
	...
	firebase.auth().onAuthStateChanged(function(user) {
		if(user != null) {
			$(placeholder).html("<img class='uihold' src='"+ user.photoURL + "' />" + user.displayName + " has started trekking time");
			record(user);
			$(cover).hide();
			getPortal(user.email, "/start", window.location.hash);
		}
		else {
	...
	firebase.auth().getRedirect
	Result();

});
function record(user) {
	firebase.database().ref("users/"+ user.uid + "/start").push({
